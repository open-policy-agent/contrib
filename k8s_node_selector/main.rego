...
import data.kuberentes.namespaces
...
#  - With nodeselector already specified <- Shouldn't be done for a namespace with a mapping
    isPod
    getPoolForNamespace(input.request.namespace)
    hasNodeSelector
...
#  - Has a mapping set for the namespace
    ...
    patch := {
        ...
        "value": {
            ...
            "agentpool": getPoolForNamespace(input.request.namespace)
        }
    }
...
getPoolForNamespace(namespace) = poolLabel {
    ...
    poolLabel = cluster_namespace.metadata.labels.agentpool
}