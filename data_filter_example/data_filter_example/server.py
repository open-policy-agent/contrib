...
def get_post(post_id):
    ...
    sql = opa.splice(
        SELECT="posts.*", FROM="posts", WHERE="posts.id=?", decision=decision
    )
...
COOKIES = ["departments", "clearance_level"]

USERS = {
    "bob": {"departments": ["sec", "dev"], "clearance_level": 5},
    "alice": {"departments": ["dev", "sec"], "clearance_level": 10},
    "charlie": {"departments": ["hr"], "clearance_level": 10},
}
